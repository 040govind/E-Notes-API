...
import com.enotes.enotes_api.dto.NotesDto.FilesDto;
...
import com.enotes.enotes_api.repositary.FavouriteNoteRepositary;
import com.enotes.enotes_api.repositary.FileRepository;
import com.enotes.enotes_api.repositary.NotesRepository;
...
public class NotesServiceImpl implements NotesService {
	...
	@Autowired
	private FavouriteNoteRepositary favouriteNoteRepo;
	...
	private FileDetails saveFileDetails(MultipartFile file) throws IOException{
          if(!ObjectUtils.isEmpty(file) && !file.isEmpty()){
			 ...
			 if(!extensionAllow.contains(extension)){
				throw new IllegalArgumentException("invalid file format ! Upload only pdf,xlsx,png,jpg");
			 }
	...
	private void checkCategoryExist(Integer category) throws Exception {
		categoryRepo.findById(category).orElseThrow(() -> new ResourceNotFoundException("category id invalid"));
	}
	...
	public void favoriteNotes(Integer noteId) throws Exception {
		...
		//System.out.println(favouriteNote);
	...
	public List<FavouriteNoteDto> getUserFavoriteNotes() throws Exception {
		...
		List<FavouriteNoteDto> favouriteNotesDto = favouriteNotes.stream().map(fn->mapper.map(fn,FavouriteNoteDto.class)).toList();
		return favouriteNotesDto;
	}
